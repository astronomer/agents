---
name: airflow-cli
description: CLI tool for interacting with Apache Airflow instances. Use when the user asks about airflow-cli commands, wants to query Airflow from the command line, needs CLI reference for health checks, DAG operations, run management, task logs, or system configuration. Useful for terminal-based workflows and scripting.
---

# Airflow CLI

A CLI tool for interacting with Apache Airflow instances. Use `airflow-cli` commands to query, manage, and troubleshoot Airflow workflows.
...
Or use CLI flags: `airflow-cli --airflow-url http://localhost:8080 --username admin --password admin <command>`
...
- `airflow` - MCP server for Airflow operations (programmatic access)
