...
The `astronomer/astro` provider manages Astro Hosted through the Platform and IAM APIs: workspaces, deployments, teams, role bindings, custom roles, and API tokens. It is under active development: required attributes, replace-on-change behavior, and enum values move between releases. Do not write HCL from memory; verify resource and attribute names against the live schema first.
...
|---|---|---|
| Organization metadata | None | `astro_organization` |
...
| Hybrid cluster ↔ workspace authorization | `astro_hybrid_cluster_workspace_authorization` | None |
| Teams (Terraform-managed only) | `astro_team` | `astro_team`, `astro_teams` |
| Team membership (managed independently of `astro_team`) | `astro_team_membership` | None |
| Role bindings for a Team that exists outside Terraform | `astro_team_roles` | None |
| Individual (non-Team) user role bindings | `astro_user_roles` | `astro_user`, `astro_users` |
| User invitations (local/no-SSO mode only) | `astro_user_invite` | None |
...
| Remote-execution agent tokens | `astro_agent_token` | None |
...
# astro_api_token.token is stored in Terraform state in plaintext (see the API-token gotcha in Step 6) - use a remote, access-controlled backend.
output "deploy_token" {
...
- **API tokens are computed, sensitive, and stored in Terraform state in plaintext.** `astro_api_token.token` has no ephemeral or write-only equivalent in this provider: creating a token in Terraform means the secret lives in state, so use a remote, access-controlled backend, or create tokens out of band (Astro UI or API) and hand them to CI/CD directly, keeping them out of state entirely.
