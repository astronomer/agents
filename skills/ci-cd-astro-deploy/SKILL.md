...
- It pytests everything in your project's `tests/` folder as part of the deploy step and fails the job (blocking the deploy) if any test fails.

Only write custom diff-detection or test-gating logic if the repository layout doesn't fit these native behaviors (for example, dags and project config split across two repositories with different CI systems).
...
Ask three questions before generating a workflow. Do not assume:

1. **Which CI system?** This skill is GitHub Actions-specific (`astronomer/deploy-action`). For GitLab CI, Jenkins, Azure DevOps, AWS CodeBuild, or Bitbucket Pipelines, use that platform's own template page under the same `ci-cd-templates/` doc tree on [astronomer.io/docs](https://www.astronomer.io/docs/astro/ci-cd-templates/template-overview) instead of adapting this skill's YAML by hand.
2. **Split dag and image deploys, or deploy both together?** The answer picks the deploy type in Step 2 (`dags-only`/`image-only` vs. `infer`/`image-and-dags`).
3. **Does the team maintain its own pre-built image in an external registry (such as GAR or ECR), separate from what this pipeline builds?** If yes, the workflow doesn't build an image at all. See the external-registry variant in Step 4's custom-image example.

Then confirm which row applies below. Do not default to single-branch/single-repository without checking; see `deploying-airflow`'s CI/CD Patterns section for the same strategic decision from the plain-CLI side if this table's GitHub Actions framing doesn't fit.
...
| Dags are business-critical; you want to test before production | **Multiple environments**: at least two permanent branches (for example, `dev`, `main`), each mapped to its own Deployment in the same Workspace | Usually still single repository, unless the next row also applies |
...
| Migrated from MWAA/Google Cloud Composer; dag authors are used to dropping files in a bucket | Either | **Dags in cloud storage**: dags pushed from an S3/GCS bucket via automation (for example, a Lambda), project config deployed from Git as an image-only deploy |
...
|---|---|---|
| `infer` (default) | Dags-only deploy if the commit only touched the dags folder; otherwise a full `image-and-dags` deploy | The default for most single-repository projects, no extra configuration needed |
| `image-and-dags` | Full project deploy: image, dags, `includes/`, `plugins/`, and any other root-level folders (for example, a `dbt/` folder) | Forcing a full deploy regardless of which files changed |
...
| `dags-only` | Only the `dags/` folder | Multi-repository or bucket-based setups where dags deploy separately from the image (requires dag-only deploy enabled on the target Deployment first, see Step 6) |
| `dbt` | Only a dbt project folder | dbt code that deploys independently of dag/image changes |

**The pytest/dag-validation gate**: for every deploy type except `image-only`, `deploy-action` runs the tests in your project's `tests/` folder before pushing code, and the job fails (blocking the deploy) if a test fails. This is the same validation `astro deploy` runs locally; you get it for free by using the action, with no separate CI job required.
...
Check the [`docker/build-push-action` docs](https://github.com/docker/build-push-action) for the current set of build inputs (for example, build-args, contexts, or SSH). That action versions independently of `deploy-action`.

**Deploying a pre-built image from an external registry (such as GAR or ECR) instead of building one in this job**: replace the build step with a pull of the existing tag (`docker pull <registry>/<image>:<tag>`, retagged if `deploy-action` expects a different local name) and pass that tag as `image-name`. `deploy-action` pushes the already-built image to Astro rather than building it. The equivalent outside `deploy-action` is calling the CLI directly: `astro deploy --image --image-name <tag>`.
...
- **`deploy-action` manages its own Astro CLI version internally.** You do not need a separate "install Astro CLI" step when using the action.

---

## Step 6: Gotchas

- **Dag-only deploy must be enabled on the target Deployment before `dags-only` or `astro deploy --dags` will work.** If it isn't, the deploy fails. Enable it per Deployment with `astro deployment update <DEPLOYMENT_ID> --dag-deploy-enabled` (see `managing-astro-deployments`) or in the Deployment settings UI. See [Enable/disable dag-only deploys](https://www.astronomer.io/docs/astro/deploy-dags#enable-or-disable-dag-only-deploys-on-a-deployment).
...
- [ ] `tests/` folder populated if the pipeline should actually block deploys on failing dag tests.
