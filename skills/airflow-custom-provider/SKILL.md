...
Package a hook, operator, or sensor for a third-party system into an independently distributed, pip-installable Airflow provider: the same mechanism every official Apache provider (such as `apache-airflow-providers-amazon` or `-snowflake`) uses. Any DAG repository can then `pip install` it and get the operators, sensors, and a custom connection type in the UI.
...
| Situation | Reach for |
|---|---|
| Integration code will be shared across 2+ DAG repositories, released independently, or installed by other teams (or the public) | A provider package (this skill) |
| A one-off integration used by a single DAG in one repository, not meant to be reused elsewhere | A plain Python class in `include/` of that repository (or a legacy `plugins/` registration); see `setting-up-astro-project` |
| A custom UI page, FastAPI endpoint, or React app embedded in the Airflow webserver | Not this skill; see `airflow-plugins` |

A sign you should have built a provider package: the same hook or operator class gets copy-pasted into a second DAG repository.
...
Folder layout, verified against the canonical template:
...
The template can drift from this listing. Before scaffolding a new provider, re-clone it and diff:
...
- **`__init__` must not reference anything only valid at task-run time.** That breaks DAG import outright, not just performance, because operators are constructed at parse time. Anything only known at task-run time belongs in a templated field (Jinja), not a constructor call.
...
- **Relaxed dependency bounds.** Pin a floor, leave the ceiling open: `depx>=2.0,<3`, not `depx==2.3.1`. This applies doubly to `apache-airflow` itself: declare a floor (for example `apache-airflow>=2.9`) and let the runtime supply the actual version. A tightly pinned or exact-match dependency is the most common cause of "provider X conflicts with provider Y" install failures.
...
- Canonical template: [astronomer/airflow-provider-sample](https://github.com/astronomer/airflow-provider-sample): folder layout, `get_provider_info()` shape, and dependency-bound guidance above were verified directly against this repository's `pyproject.toml`, `__init__.py`, and README. Re-clone it if anything here looks stale; it is the source of truth, not this file.
